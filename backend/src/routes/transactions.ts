...
export const setupTransactionRoutes = (app: Express) => {
  ...
  app.post('/transactions', authMiddleware, async (req: AuthRequest, res: Response, next: NextFunction) => {
    try {
      const { items } = z.object({
        items: z.array(
          z.object({
            book_id: z.string().or(z.number()).transform(val => 
  ...
  // GET /transactions/:transaction_id - Get detail transaksi
  ...
  // GET /transactions/statistics - Get statistik penjualan
    ...
    try {
      ...
      const totalAmount = orders.reduce((sum, order) => {
        const orderTotal = order.order_items.reduce(
          (itemSum, item) => itemSum + Number(item.book.price) * item.quantity,
          0
        );
      ...
      const mostPopularGenre = sortedGenres.length > 0 ? sortedGenres[0][0] : null;
      const leastPopularGenre = sortedGenres.length > 0 ? sortedGenres[sortedGenres.length - 1][0] : null;
